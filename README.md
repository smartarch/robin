...
## Installation
<b>Robin</b> is written in `python-django` which is a python package, and can be installed in the local library. To skip the installation section and use a docker, refer to the [Docker](/container). This section covers the following sections:
...
<i>NOTE that since the `Debug=True` at this moment, entering a wrong DOI will show as an exception instead of formal bad request</i>
